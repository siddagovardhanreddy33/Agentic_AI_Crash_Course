...
"""
reviews API - reads from the 'reviews' table in the store.db and returns aggregated rating information for products.
...
DB_PATH = os.path.join(os.path.dirname(__file__) , "store_expanded.db")


def get_product_rating(product_id:int) -> dict:
    """ Return avaerage rating and review count for a single product . """
...
def get_ratings_for_products(product_ids:list[int])-> list[dict]:
    """ return ratings for a list of produvt IDs >"""
    if not product_ids:
        return []
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
